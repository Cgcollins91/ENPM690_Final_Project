...
class TD3Config:
    ...
    hidden_dim                      : int              = 256  # integer hidden dim value tracked by t d3 config
    actor_lr                        : float            = 3e-4  # actor optimizer learning rate
    critic_lr                       : float            = 3e-4  # critic optimizer learning rate
    gamma                           : float            = 0.995  # discount factor used by TD3 updates
    tau                             : float            = 0.005  # target-network interpolation factor for TD3 updates
    policy_delay                    : int              = 2  # integer policy delay value tracked by t d3 config
    policy_noise                    : float            = 0.2  # floating-point policy noise value used by t d3 config
    policy_noise_finger             : float            = 0.0  # floating-point policy noise finger value used by t d3 config
    noise_clip                      : float            = 0.5  # floating-point noise clip value used by t d3 config
    exploration_noise               : float            = 0.25  # floating-point exploration noise value used by t d3 config
    exploration_noise_finger        : float            = 0.0  # floating-point exploration noise finger value used by t d3 config
    target_q_clip                   : float            = -1.0  # floating-point target q clip value used by t d3 config
    critic_grad_clip                : float            = -1.0  # floating-point critic grad clip value used by t d3 config
    actor_pre_tanh_l2               : float            = 0.0  # floating-point actor pre tanh l2 value used by t d3 config
    observation_normalization       : bool             = False  # boolean value indicating the observation normalization state for t d3 config
    obs_norm_eps                    : float            = 1e-5  # floating-point obs norm eps value used by t d3 config
    obs_norm_clip                   : float            = 10.0  # floating-point obs norm clip value used by t d3 config
    reward_normalization            : bool             = False  # boolean value indicating the reward normalization state for t d3 config
    reward_norm_eps                 : float            = 1e-5  # floating-point reward norm eps value used by t d3 config
    reward_norm_clip                : float            = 10.0  # floating-point reward norm clip value used by t d3 config
    actor_freeze_steps              : int              = 0  # step count used for actor freeze steps scheduling or reporting
    rl_actor_freeze_until_train_step: int              = -1  # step count used for rl actor freeze until train step scheduling or reporting
    bc_only_steps                   : int              = 0  # step count used for bc only steps scheduling or reporting
    bc_only_weight                  : float            = 0.0  # weight applied to bc only terms
    bc_only_arm_weight              : float            = -1.0  # weight applied to bc only arm terms
    bc_only_finger_weight           : float            = -1.0  # weight applied to bc only finger terms
    ...
    teacher_bc_decay_steps          : int              = 0  # step count used for teacher bc decay steps scheduling or reporting
    actor_q_action_gate_mode        : str              = "env"  # string actor q action gate mode value used by t d3 config
    actor_bc_action_gate_mode       : str              = "env"  # string actor bc action gate mode value used by t d3 config
    finger_noise_bypass_unlock      : bool             = False  # boolean value indicating the finger noise bypass unlock state for t d3 config
    debug_nonfinite_updates         : bool             = False  # boolean value indicating the debug nonfinite updates state for t d3 config
    stop_on_nonfinite_update        : bool             = False  # boolean value indicating the stop on nonfinite update state for t d3 config
    training_phase_id               : int              = 0  # integer training phase id value tracked by t d3 config
    active_n_step                   : int              = 1  # step count used for active n step scheduling or reporting
    active_updates_per_step         : int              = 1  # step count used for active updates per step scheduling or reporting
    gate_config                     : ActionGateConfig = field(  # stores gate config for t d3 config
