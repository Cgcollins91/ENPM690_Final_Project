# === File Documentation =====================================================
# Path: tasks/common_observations/dex3_state.py
# Summary: Implements dex3 state.
...
This module defines utilities for extracting and managing the state of the Dex3 robot, particularly focusing on the gripper joint states.
It includes functions to retrieve the relevant joint names and to gather the joint positions, velocities, and torques for the gripper.
Additionally, it integrates with the DDS communication system to publish the gripper joint states for external monitoring
or control purposes. The implementation includes caching mechanisms to optimize the retrieval of joint states and minimize overhead 
when publishing to DDS. This setup is designed to be compatible with the local task utilities and can be easily extended for more complex
...
def get_robot_girl_joint_names() -> list[str]:
    """Return robot girl joint names for the active configuration or simulation state."""
    return [
        # hand joints (14)
        # left hand (7)
...
def _get_dex3_dds_instance():
    ...
    global _dex3_dds, _dds_initialized, _dds_cleanup_registered
...
def get_robot_dex3_joint_states(
    ...
    if enable_dds and dds_manager is not None and len(pos_buf) > 0:
        try:
            import time
