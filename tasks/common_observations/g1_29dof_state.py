# === File Documentation =====================================================
# Path: tasks/common_observations/g1_29dof_state.py
# Summary: Implements g1 29dof state.
# Primary Purpose: Supports task registration and environment configuration, observation composition and sensor/state feature extraction.
# Project Component: tasks/common_observations.
# Behavior Notes:
...
This module defines the observation composition and sensor/state feature extraction for the Unitree G1 robot with 29 degrees of freedom (DOF).
It includes functions to retrieve joint names and states, compute IMU data, and manage a DDS communication instance for publishing robot state 
information. The `get_robot_boy_joint_states` function retrieves joint positions, velocities, and torques, while the `get_robot_imu_data`
function computes proper IMU readings by differentiating velocity to get acceleration and compensating for gravity. 
The module also includes caching mechanisms to optimize performance by reducing redundant computations and managing shared memory for efficient 
data access. This setup is designed to be compatible with the local task utilities and can be easily extended for more complex observation 
compositions or additional sensor integrations as needed by the project. The code also includes safeguards against non-finite values in the 
joint state data, ensuring that observations remain numerically stable for downstream consumers such as policies and DDS subscribers.
"""
...
def get_robot_boy_joint_names() -> list[str]:
    """Return robot boy joint names for the active configuration or simulation state."""
    return [
        # leg joints (12)
        # left leg (6)
...
def get_robot_arm_joint_names() -> list[str]:
    """Return robot arm joint names for the active configuration or simulation state."""
    return [
        # arm joints (14)
        # left arm (7)
        "left_shoulder_pitch_joint",
...
def _get_g1_robot_dds_instance():
    ...
    global _g1_robot_dds, _dds_initialized, _dds_cleanup_registered

    if dds_manager is None:
        _dds_initialized = True
        return None

    if not _dds_initialized or _g1_robot_dds is None:
        try:
            # Avoid noisy "not found" logs during the first env reset before DDS registration.
            ...
            if _g1_robot_dds is not None:
                ...
                if not _dds_cleanup_registered:
                    ...
                    def cleanup_dds():
                        """Shut down the DDS participant used by robot state subscription."""
                        try:
                            if _g1_robot_dds:
...
def get_robot_boy_joint_states(
    ...
    # PhysX occasionally reports transient NaN/Inf values in actuator state
    # buffers; clamp them here so policy observations and DDS messages stay
    # numerically safe for downstream consumers.
    pos_finite = bool(torch.isfinite(pos_buf).all().item())
    vel_finite = bool(torch.isfinite(vel_buf).all().item())
    torque_finite = bool(torch.isfinite(torque_buf).all().item())
    if not (pos_finite and vel_finite and torque_finite):
        if _obs_cache["nonfinite_warn_count"] < 5:
            print(
                f"[g1_state] Non-finite joint-state values detected "
                f"(pos={int(not pos_finite)} vel={int(not vel_finite)} torque={int(not torque_finite)}); "
                f"replacing with 0.",
            )
        _obs_cache["nonfinite_warn_count"] += 1
        pos_buf.nan_to_num_(nan=0.0, posinf=0.0, neginf=0.0)
        vel_buf.nan_to_num_(nan=0.0, posinf=0.0, neginf=0.0)
        torque_buf.nan_to_num_(nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if enable_dds and dds_manager is not None and combined_buf.shape[0] > 0:
        try:
            import time
